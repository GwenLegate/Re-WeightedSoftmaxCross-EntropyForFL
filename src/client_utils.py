...
def parallel_mask(args, logits, labels):
    """
    ******** DEPRECIATED CODE FOR OLD HARD MASKING TECHNIQUE ******
    Masks logits of data classes not present in client dataset
    Args:
        logits: output of the client model forward pass
    Returns logits masked so only classes present in the client dataset are non zero
    """
    og_shp = logits.size()
    logits = logits.reshape(max(int(args.num_clients * args.frac), 1), -1, logits.size(-1))
    mask = torch.zeros_like(logits, dtype=torch.bool)
    for client_idx, client_label in enumerate(labels):
        mask[client_idx, :, client_label.unique()] = True

    return logits.masked_fill(~mask, -1e9).view(og_shp)

def parallel_weighted_log_softmax(args, logits, alphas):
    """
    Weighted log softmax so classes are scaled according to their proportions
    Args:
        logits: output of the client model forward pass
        alphas: proportions of the classes for each selected client

    Returns weighted log softmax so only classes are scaled according to their proportions
    """
    og_shp = logits.size()
    logits = logits.reshape(max(int(args.num_clients * args.frac), 1), -1, logits.size(-1))
    alphas = torch.FloatTensor(alphas).to(args.device)
    log_alphas = alphas.log().clamp_(min=-1e9)
    for i in range(max(int(args.num_clients * args.frac), 1)):
        deno = torch.logsumexp(log_alphas[i] + logits[i, :, :], dim=-1, keepdim=True)
        logits[i, :, :] = log_alphas[i] + logits[i, :, :] - deno

    return logits.view(og_shp)
